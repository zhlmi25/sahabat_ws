...
//Whether it is within the filtering range
    ...
    full_scan_data_st &out)
{
    ...
    int count = in.vailtidy_point_num; //Number of points in a circle
    int window_size = 5;
    ...
    for (int i = (window_size / 2); i < (count - (window_size / 2)); i++)
    {
        if(isValidRange(ParaInf, in.data[i].distance))
        {
            ...
            {
                if (in.data[i + j - 2].distance == 0) // zero value does not participate in averaging
                    continue;

                if (abs(in.data[i].distance - in.data[i + j - 2].distance) < dis_threshold) //Only points within the threshold range participate in averaging
                {
                    cnt++;
    ...
    full_scan_data_st &out)
{
    ...
    int circle_len = in.vailtidy_point_num; //Number of points in a circle
    ...
    //Range core
    for (i = w_r; i < circle_len + 4 - w_r; i++) 
    {
        current_data = temp_dists_data[i]; 
    ...
    full_scan_data_st &out)
{
	...
	case 0:
		TanAngleThreshold = 0.140540834702391; //8°
		PointNumThreshold = 2;
		break;
	case 1:
		TanAngleThreshold = 0.267949192431123; //15°
		PointNumThreshold = 2;
		break;
	case 2:
		TanAngleThreshold = 0.324919696232906;  //18°
		PointNumThreshold = 2;
		break;
	case 3:
		TanAngleThreshold = 0.363970234266202;//20°
		PointNumThreshold = 1;
		break;
	}
